...
一番の理由は、自分で作った syntax の面倒を自分で見たかったからです。
...
なので、macro の入力そのものを editor 上で見て、そこに対して diagnostics を出したい。

こういうのは LSP でやるのがよさそう。
...
理由は単純で、v1 では async runtime を持ち込みたくなかったからです。やることは document を受け取って text を保持して、hover request が来たら source を scan して返すだけ。これに Tokio を立てるのはちょっと大げさかなと思った。
...
じゃあ LSP でもそれを使えばいいじゃん、という話になるんだけど、v1 ではやりませんでした。

理由は、LSP が欲しい情報と proc-macro が欲しい情報が違うからです。
...
最初は message loop を抜けたあとにそのまま `io_threads.join()` していました。すると `shutdown` を送っても process が終わらない。

原因は `Connection` を持ったまま join していたことです。sender/receiver が生きているので、IO thread 側が終われない。先に `drop(connection)` してから join する必要がありました。
...
あともう一つ、`initialize` response も一回間違えました。
...
`capabilities.capabilities` になっている。ひどい。(´･\_･`)

正しくは `ServerCapabilities` を渡します。

```rust
connection.initialize(serde_json::to_value(capabilities).unwrap())?;
```

こういうのは `cargo check` では当然わかりません。実際に `Content-Length` frame を組んで `initialize`、`didOpen`、`hover`、`shutdown` を流して確認しました。地味だけど、こういう確認は大事。
